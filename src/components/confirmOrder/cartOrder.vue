	...
	<div class="order-wrap">
		...
		<div class="price-set-wrap"  >
			<div class="total price clearfix">
				...
				<span class="fr" v-if="status !=1">￥{{total_price}}</span>
				<span class="fr" v-else="status ==1">￥{{parseFloat(total_price)+parseFloat(money)}}</span>
			</div>
			...
			<div class="footer clearfix">
				...
				<div class="fr money">
					实付款&nbsp;:&nbsp;<span class="price">￥
						<span v-if="status ==1">{{parseFloat(all_price)+parseFloat(money)|keepTwoNum}}</span>
						<span v-else="status !=1">{{all_price|keepTwoNum}}</span>
					</span>
				</div>
			</div>
		</div>
		...
		methods: {
			getCartOrder(){
				...
				for(var i=0;i<this.store.length;i++){
					...
					store_id=this.store[i].id;
					console.log(this.store[i].id);
				}
				...
				this.axios.post(this.$httpConfig.goBuy, qs.stringify({
					...
					if(res.data.status == 1){
						...
						this.$router.push({
								...
								params: {
									id: 1 ,//1为商品 3为积分兑换
									number : this.all_price + Number(this.money)
								},
							});
