<template>
  <div class="product szcf">
    ...
    <div class="describe" v-if="$store.state.commodity_data">
      ...
      <p class="timer">{{ cut_time }}<b class="gray">，以后购买</b></p>
      <div class="price active">
        <p class="orc clearfix">
          <em class="icon fl"></em>
          <em class="fl">{{ $store.state.commodity_data.goods.integral }}</em>
        </p>
        <p class="new clearfix">
          还需支付: {{ $store.state.commodity_data.goods.money }}元
        </p>
        <p class="new">
          市场参考价：<s>{{ $store.state.commodity_data.goods.price }}</s
        >元
        </p>
        <p class="new clearfix">
          stock: {{ $store.state.commodity_data.goods.stock }}
        </p>
      </div>
    </div>
    <!-- 已选 -->
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.$store.state.integralType = this.$route.query.type;
      this.$store.state.integralID = this.$route.params.id;
    ...
    methods: {
      ...
      ax() {
        this.axios({
          url: this.$httpConfig.integralGoodInfo,
          method: "get",
    ...
    mounted() {
      ...
      this.axios
              .post(
                      this.$httpConfig.integralGoodInfo,
                      qs.stringify({
                        id: this.$route.params.id
                      })
              )
