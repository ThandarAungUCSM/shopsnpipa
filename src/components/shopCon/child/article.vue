<template>
	<div>
		<ul class="recommendList" v-if="temList ==0" v-for="(goods, index) in recommondGoods" :key="index">
			<li v-for="(item, i) in goods" :key="i"  @click="enterInto(item.id)">
		...
		<!--<ul class="recommendList" v-if="temList ==0">
			<li v-for="(i, index) in hotList" :key="index"  @click="enterInto(i.id)">
				&lt;!&ndash;				{{item}}&ndash;&gt;
...
export default {
	...
	mounted() {
        ...
        this.getReGoods();
        setTimeout(this.getReGoods, 800);
        $(window).scroll(function() {
            ...
            if (
                this.isBottom == false &&
                bot + $(window).scrollTop() >= 310 + $(".recommendList")[0].clientHeight
            ) {
	...
	methods: {
        ...
        getReGoods() {
            ...
            else
            {
                ...
                var params = { store_id: this.id, page: this.page };
                this.$HTTP(this.$httpConfig.getRecommondGoods, params, "post")
                    .then(res => {
                        this.recommondGoods.push(res.data.data);
                        this.page++;
                        this.isBottom = false;
                        this.isEnd = false;
                        // this.getAd();
