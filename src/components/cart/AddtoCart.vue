<template>
    <div class="goos" ref="cart">
        ...
        <div style="margin-bottom:1.5rem" v-show="nav[0].isSelected">
            <dl
                    class="mark-wrap"
                    v-if="$store.state.cart_data"
                    v-for="(items,index1) in $store.state.cart_data"
                    v-bind:key="index1"
            >
                <dt>
                    <label class="checkBox" :class="{active:items.checked}" @click="addShop(index1,items)">
                        <span></span>
                    </label>
            ...
            <em class="fl"></em>
                ...
                >
                    <label
                            ...
                            @click="addCom(index1,index,item)"
                            checked
                    >
                        <span></span>
                    </label>
    ...
    export default {
        ...
        mounted() {
            ...
            window.addEventListener("scroll", function() {
                ...
                console.log(
                    ...
                    _this.roll_switch == false,
                    _this.slide_switch == true
                );
                if (
                    document.body.scrollTop + window.innerHeight >=
                    document.body.offsetHeight
        ...
        methods: {
            ...
            cartList(){
                ...
                this.cart_list = this.$store.state.cardList;
                console.log(this.cart_list);
            },
            selectNav(index) {
                console.log(index);
            ...
            getPackageCart() {
                ...
                // 获取购物车套餐商品
                this.axios
                    .post(this.$httpConfig.cartPackageList)
                    .then(res => {
            ...
            addShop(index, items) {
                ...
                this.$store.state.cart_data[index]["checked"] = !this.$store.state
                    .cart_data[index]["checked"];
                this.getAllprice();
                this.hasAllchecked();
            },
            addCom(index1, index, item) {
                //单选
                var list = this.$store.state.cart_data[index1]["goods"];
                var len = list.length;
                if (list[index]["checked"]) {
                    ...
                    off == true
                        ? (this.$store.state.cart_data[index1]["checked"] = true)
                        : (this.$store.state.cart_data[index1]["checked"] = false);
            ...
            hasAllchecked() {
                ...
                for (var i = 0, len = this.$store.state.cart_data.length; i < len; i++) {
                    if (
                        typeof this.$store.state.cart_data[i]["checked"] === "undefined" ||
                        this.$store.state.cart_data[i]["checked"] == false
                    ) {
                        flag1 = false;
                        break;
                    }
            ...
            toOrder() {
                ...
                for (var i = 0; i < this.goods.length; i++) {
                    ...
                    if (newArr.indexOf(this.goods[i].store_id) == -1) {
                        newArr.push(this.goods[i].store_id);
                    }
                }

                console.log(newArr);
                //通过双层for循环 比较是否有相同的店铺id
...
<style lang="less" scoped>
    .goos {
        ...
        .nav {
            ...
            li.active {
                color: #00c65d;
                border-bottom: 0.03rem solid #00c65d;
            }
        }
        .footer-wrap {
            position: absolute;
        ...
        .cart-active {
            ...
            .text {
                ...
                a {
                    font-size: 0.32rem;
                    color: #00c65d;
                    border-bottom: 1px solid #00c65d;
